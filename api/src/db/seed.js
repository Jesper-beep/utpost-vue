...
const run = async () => {
	await createTables();
	await pool.query(
		"truncate photos, tour_logs, tours, guides, users restart identity",
	);
	...
	for (let i = 0; i < 200; i++) {
       ...
       values ($1,$2,$3,$4,$5,$6) returning id`,
			[
				pick(userIds),
				Math.random() > 0.25 ? pick(guideIds) : null,
				`Tur ${i + 1}`,
				started,
				2000 + rnd(30000),
				Math.random() > 0.6
					? "Blött väder, mycket folk vid rastplatsen."
					: null,
			],
		);
		...
		for (let p = 0; p < points; p++) {
			...
			params.push(
				tourId,
				new Date(started.getTime() + p * 300000),
				58 + Math.random() * 10,
				12 + Math.random() * 8,
				100 + rnd(900),
				90 + rnd(70),
	...
	console.log(
		`Seed klar: 12 användare, 40 guider, 200 turer, ${logCount} loggpunkter.`,
	);
	await pool.end();
};
